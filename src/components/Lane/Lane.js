...
import MiniModalWrapper from './MiniModalsWrapper';
import './Lane.scss';
...
const Lane = ({ children }) => {
  ...
  const midLane =
    movies &&
    keyedMovies.map((movie) => {
      return (
        <LaneItem key={movie.key} updateZIndexRef={updateZIndexRef}>
          {/* heb er een component van gemaakt die ze eigen state bijhoudt */}
          <MiniModalWrapper size={size}>
            <MiniModal />
          </MiniModalWrapper>
